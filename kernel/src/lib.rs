...
"spouse": null}"#;
    ...
    fn transform_multi() {
        let mut s = DefaultSpace::new();
        let file_path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../data/simpsons.metta");
...
(axiom (= (L $x $x $x) (* (K $x (\ $x 1)) $x)))"#;
    ...
    fn subsumption_big() {
        let mut s = DefaultSpace::new();
        let file_path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../data/big.metta");
